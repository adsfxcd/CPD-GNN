"""
Training script for CPD-GNN (Cross-Modal Prompt Disentangled Graph Neural Network)
for incomplete multimodal conversational emotion recognition.
This script supports the GraphModelWithPrompt model with:
1. Unified semantic projection and shared-private feature disentanglement
2. Cross-modal prompt-based information compensation
3. Dynamic modality balancing under missing-modality conditions
4. Local contextual interaction and higher-order conversational dependency modeling
"""
...
def get_loaders(audio_root, text_root, video_root, num_folder, dataset, batch_size, num_workers, seed):
    ...
    if dataset in ['CMUMOSI', 'CMUMOSEI']:
        ...
        trainNum = len(dataset_obj.trainVids)
        valNum = len(dataset_obj.valVids)
        testNum = len(dataset_obj.testVids)
        ...
        val_loader = DataLoader(dataset_obj, batch_size=batch_size,
                                sampler=SubsetRandomSampler(val_idxs),
                                collate_fn=dataset_obj.collate_fn,
                                num_workers=num_workers, pin_memory=False)
        test_loader = DataLoader(dataset_obj, batch_size=batch_size,
        ...
        adim, tdim, vdim = dataset_obj.get_featDim()
        return train_loaders, val_loaders, test_loaders, adim, tdim, vdim

    if dataset in ['IEMOCAPFour', 'IEMOCAPSix']:
        dataset_obj = IEMOCAPDataset(label_path=path.PATH_TO_LABEL_Win[dataset],
                                     audio_root=audio_root,
                                     text_root=text_root,
                                     video_root=video_root)
        ...
        session_to_idx = {}
        for idx, vid in enumerate(dataset_obj.vids):
            session = int(vid[4]) - 1
            if session not in session_to_idx:
                session_to_idx[session] = []
            session_to_idx[session].append(idx)
        assert len(session_to_idx) == num_folder, f'Must split into {num_folder} folders'
...
def build_model(args, adim, tdim, vdim):
    ...
    if args.use_prompt:
        ...
        print(f'Using GraphModelWithPrompt (prompt_dim={args.prompt_dim}, n_proto={args.n_proto})')
    else:
        model = GraphModel(
            args.base_model,
            adim, tdim, vdim, D_e, graph_h,
        ...
        print('Using original GraphModel')

    print(f"Model has {sum(x.numel() for x in model.parameters())} parameters in total")
    print(f'Graph NN with {args.base_model} as base model.')
    return model


def generate_inputs(audio_host, text_host, visual_host, audio_guest, text_guest, visual_guest, qmask):
    """Generate input features based on speaker mask"""
    input_features = []
...
def random_mask(view_num, input_len, missing_rate):
    """Randomly generate incomplete data information"""
...
def train_or_eval_model(args, model, reg_loss, cls_loss, rec_loss, dis_loss, dataloader,
                        mask_rate=None, optimizer=None, train=False):
    """Training or evaluation loop with Prompt-based model support"""
    ...
    for data in dataloader:
        ...
        if not lower_bound:
            ...
            host_mask = torch.logical_and(torch.logical_and(audio_host_mask, text_host_mask), visual_host_mask).int()
        ...
        # Forward pass
        if use_prompt:
            # Use new model with prompt and disentangle features
            if reccls_flag:
                outputs = model(masked_input_features, qmask, umask, lengths, return_disentangle=True)
                _, recon_input_features, _, _, _, _, _, _ = outputs
                outputs = model([recon_input_features[0]], qmask, umask, lengths, return_disentangle=True)
                log_prob, _, hidden, shared_features, private_features, log_prob_a, log_prob_t, log_prob_v = outputs
            else:
                outputs = model(masked_input_features, qmask, umask, lengths, return_disentangle=True)
                log_prob, recon_input_features, hidden, shared_features, private_features, log_prob_a, log_prob_t, log_prob_v = outputs
        else:
            # Use original model
            if reccls_flag:
        ...
        for ii in range(len(tempseqlen)):
            ...
            itemfmask = tempfmask[ii][:int(tempseqlen[ii]), :]
            itemlabel = templabel[ii][:int(tempseqlen[ii])]
            itemspks = tempqmask[ii][:int(tempseqlen[ii])]
        ...
        # Total loss
        if args.loss_recon:
            loss = (1 - args.recon_loss_weight - args.disentangle_weight) * loss1 + \
                   args.recon_loss_weight * loss2 + \
                   args.disentangle_weight * loss3
        else:
            loss = (1 - args.disentangle_weight) * loss1 + args.disentangle_weight * loss3
    ...
    return avg_accuracy, avg_fscore, vidnames, [avg_loss, avg_loss1, avg_loss2, avg_loss3], \
           [savepreds, savelabels, savespeakers, savehiddens, savefmask]
...
if __name__ == '__main__':
    ...
    # Params for model
    parser.add_argument('--base-model', type=str, choices=['LSTM', 'GRU'], default='GRU',
                        help='base recurrent model')
    parser.add_argument('--time-attn', action='store_true', default=False,
                        help='whether to use nodal attention in graph model')
    parser.add_argument('--windowp', type=int, default=3,
                        help='context window size for past utterances')
    parser.add_argument('--windowf', type=int, default=3,
                        help='context window size for future utterances')
    ...
    parser.add_argument('--prompt-dim', type=int, default=16,
                        help='dimension of cross-modal prompts')
    parser.add_argument('--n-proto', type=int, default=10,
                        help='number of prototypes for prompt generation')
    parser.add_argument('--max-seq-len', type=int, default=150,
                        help='maximum sequence length for PromptFormer')
    parser.add_argument('--disentangle-weight', type=float, default=0.1,
                        help='weight for disentangle loss')
    ...
    parser.add_argument('--num-folder', type=int, default=5, help='folders for cross-validation')
    parser.add_argument('--seed', type=int, default=55, help='random seed')
    parser.add_argument('--mask-type', type=str, default='constant-0.1',
                        help='mask rate for input augmentation')
    parser.add_argument('--loss-recon', action='store_true', default=False,
                        help='whether to use reconstruction loss')
    parser.add_argument('--recon-loss-weight', type=float, default=0.3,
                        help='weight for reconstruction loss')
    parser.add_argument('--reccls-flag', action='store_true', default=False,
    ...
    print(f"Using Prompt: {args.use_prompt}")
    print(f'\n========== Reading Data ==========\n')
    ...
    assert len(train_loaders) == args.num_folder, f'Error: folder number mismatch'
    ...
    for ii in range(args.num_folder):
        ...
        reg_loss = MaskedMSELoss()  # MSE Loss for regression
        ...
        for epoch in range(args.epochs):
            ...
            train_acc, train_fscore, train_names, train_loss, trainsave = train_or_eval_model(
                args, model, reg_loss, cls_loss, rec_loss, dis_loss, train_loader,
                mask_rate=mask_rate, optimizer=optimizer, train=True
            )
            val_acc, val_fscore, val_names, val_loss, valsave = train_or_eval_model(
                args, model, reg_loss, cls_loss, rec_loss, dis_loss, val_loader,
                mask_rate=mask_rate, optimizer=None, train=False
            )
            test_acc, test_fscore, test_names, test_loss, testsave = train_or_eval_model(
                args, model, reg_loss, cls_loss, rec_loss, dis_loss, test_loader,
                mask_rate=mask_rate, optimizer=None, train=False
            )
            ...
            test_disentangle.append(test_loss[3])
            all_losses.append({
    ...
    print(f'Mean Disentangle Loss: {mean_disentangle:.4f}')
